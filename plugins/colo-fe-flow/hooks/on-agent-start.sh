...
cff_log_append_runtime "$PROJECT_ROOT" "agents" "AGENT_START ${AGENT_NAME:-unknown} active_ticket=${ACTIVE_TICKET:-none}"

if [[ -n "$ACTIVE_TICKET" ]]; then
  cff_log_append "$PROJECT_ROOT" "$ACTIVE_TICKET" "AGENT_START ${AGENT_NAME:-unknown}"
fi
